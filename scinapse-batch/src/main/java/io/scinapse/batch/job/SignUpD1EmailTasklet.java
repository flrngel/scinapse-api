...
public class SignUpD1EmailTasklet implements Tasklet {

    @Value("#{jobParameters[target_date]}")
    private String targetDate;

    @Value("${pluto.server.email.sg.template.sign-up-d1}")
    private String signUpD1EmailTemplate;
    ...
    public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws ParseException {
        ...
        helper.sendSlackAlarm("`" + Arrays.toString(profiles) + "` *Sign Up D1* Sending mails to " + members.size() + " users...");
        ...
        helper.sendSlackAlarm("`" + Arrays.toString(profiles) + "` *Sign Up D1* Sent " + counter + " mails successfully!!!");
    ...
    private void sendEmail(Member member) {
        Mail mail = new Mail();
        mail.addCategory("sign-up-d1");
        mail.setTemplateId(signUpD1EmailTemplate);
